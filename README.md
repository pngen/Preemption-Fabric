...
The runtime models priority-inversion evidence without becoming a scheduler: low-priority execution holding reclaimable resources, higher-priority work blocked on those resources, preemption cost, time/work to the next safe point, estimated recomputation, state-capture cost, release cost, resume cost, and non-preemptible interval. It exposes an inspectable assessment; the caller decides whether to request preemption. Preemption Fabric answers whether the requested preemption is feasible and what it costs/risks.
