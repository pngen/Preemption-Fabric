// Basic cooperative preemption example (in-process, reference adapters).
// Demonstrates the full safe-preemption protocol: request -> defer to safe point
// -> quiesce -> durable state -> release -> PREEMPTED -> fresh resume -> RESUMED.
#include <cstdio>
...
int main() {
  ...
  // Work is between boundaries: defer.
  rt.begin_quiesce();
  ...
  // The old attempt is fenced: it cannot publish authoritative progress now.
  std::printf("outcome publish_progress while fenced=%d\n", (int)rt.publish_progress(900, ProgressKind::kDurable));
  ...
  // Fresh authority resumes from the preserved boundary, not from zero.
  rt.publish_progress(600, ProgressKind::kDurable);
  std::printf("durable progress after resume=%llu\n", (unsigned long long)rt.durable_progress());
  return 0;
}
